 ...
 const [annoucements, setAnnoucements] = useState([]);
  useEffect(() => {
    const fetchData = async () => {
      try {
        const AnnoucementsResponse = await axios.get("http://localhost:8080/pengumuman");
    ...
    <main className="font-poppins">
      ...
      <div className="my-12 max-w-7xl px-4 lg:mx-auto lg:px-6">
        ...
        <div className="divide-y divide-solid divide-gray-400">
          {annoucements.map((annoucementsItem, ID) => (
            <Notice
              key={ID}
              subtitle={stripTags(annoucementsItem.descNotice)}
            />
          ))}
        </div>
      </div>
